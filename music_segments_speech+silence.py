...
for i in range(len(speech_frames)):
    try:
        ...
        if i == 0:
            if start != 0:
                slice_audios.append(wav_src_all[start : speech_frames[i+1]["start"]])
        else: # normal samples
            ...
            if i != len(speech_frames)-1:
              ...
              slice_audios.append(wav_src_all[start:end])
...
for i in range(len(slice_audios)):
  ...
  write(save_path + 'segment_' + str(i) + '.wav', sr_audio_original, segmented_audio)